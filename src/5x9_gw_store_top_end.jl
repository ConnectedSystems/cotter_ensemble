...
@everywhere function run_state_calibration(metric)
    ...
    outfile = "$(calib_param_path)top-end_log_$(state_str)_$(approach).txt"
    open(outfile, "w") do f
        print(f, join(params, ","))
    end
    ...
    savefig(joinpath(calib_fig_path, "burn-in_multi_restricted_top-end_$(state_str)_$(approach)_calibration.png"))

    qqplot(calib_data, calib_sim, title="Multi-Obj Restricted Top End Calibration\n($(state_str); $approach)")
    xlabel!("Historic")
    ylabel!("Simulated")
    savefig(joinpath(calib_fig_path, "restricted_top-end_$(state_str)_$(approach)_qq_calib.png"))
    ...
    savefig(joinpath(calib_fig_path, "restricted_top-end$(state_str)_$(approach)_validation.png"))

    qqplot(valid_data, valid_sim, title="Multi-Obj Restricted Top End Validation\n($(state_str); $approach)")
    xlabel!("Historic")
    ylabel!("Simulated")
    savefig(joinpath(calib_fig_path, "restricted_top-end_$(state_str)_$(approach)_qq_valid.png"))
end
