...
@everywhere function run_state_calibration(metric)
    ...
    outfile = "$(calib_param_path)restricted_log_$(state_str)_$(approach).txt"
    open(outfile, "w") do f
        print(f, join(params, ","))
    end
    ...
    xlabel!("Historic")
    ylabel!("Simulated")
    savefig(joinpath(calib_fig_path, "restricted_log_$(state_str)_$(approach)_qq_calib.png"))
    ...
    savefig(joinpath(calib_fig_path, "restricted_log_$(state_str)_$(approach)_validation.png"))

    qqplot(valid_data, valid_sim, title="4-parameter log Validation\n($(state_str); $approach)")
    xlabel!("Historic")
    ylabel!("Simulated")
    savefig(joinpath(calib_fig_path, "restricted_log_$(state_str)_$(approach)_qq_valid.png"))
end
